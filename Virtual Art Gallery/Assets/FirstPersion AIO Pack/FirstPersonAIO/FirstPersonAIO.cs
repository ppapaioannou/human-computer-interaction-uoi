...
public class FirstPersonAIO : MonoBehaviour {
    ...
    private void OnCollisionEnter(Collision CollisionData){
        for(int i = 0; i<CollisionData.contacts.Length; i++){
                float a = Vector3.Angle(CollisionData.contacts[i].normal, Vector3.up);
                if(CollisionData.contacts[i].point.y <  transform.position.y - ((capsule.height/2) - capsule.radius*0.95f)){
                    ...
                    if(advanced.maxSlopeAngle>0){
                        ...
                        advanced.curntGroundNormal = CollisionData.contacts[i].normal;
                    }
    ...
    public class FPAIO_Editor : Editor{
        ...
        public override void OnInspectorGUI(){
            if(t.transform.localScale!=Vector3.one){
                ...
                Debug.LogWarning("Scale needs to be (1,1,1)! \n Please scale controller via Capsule collider height/radius.");
            }
            SerT.Update();
